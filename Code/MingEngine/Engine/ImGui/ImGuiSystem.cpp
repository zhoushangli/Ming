...
namespace
{
std::string const kImGuiIniFilePath = "res://.ming/Editor/EditorLayout.ini";
...
void ApplyGodotImGuiTheme()
{
	...
	io.Fonts->AddFontFromMemoryCompressedTTF(
		JetBrainsMonoFont::GetData(),
		static_cast<int>(JetBrainsMonoFont::GetSize()),
		24.f);
}
} // namespace
...
void ImGuiSystem::Startup()
{
	...
	std::filesystem::path iniFilePhysicalPath;
	if (!g_engine->m_fileSystem->TryGetPhysicalPath(kImGuiIniFilePath, iniFilePhysicalPath))
	{
		return;
	}

	// TODO: We need to have a universal entry to create and ensure path
	std::filesystem::create_directories(iniFilePhysicalPath.parent_path());
	m_iniFilePath = iniFilePhysicalPath.string();
	...
	if (std::filesystem::exists(iniFilePhysicalPath))
	{
	...
	bool result = g_engine->m_renderServer->InitImGui();
	GUARANTEE_OR_DIE(result, "Failed to initialize ImGui renderer backend.");
}

void ImGuiSystem::Shutdown()
{
	g_engine->m_renderServer->ShutdownImGui();
...
{
	g_engine->m_renderServer->BeginImGuiFrame();
...
void ImGuiSystem::EndFrame()
{
	...
	g_engine->m_renderServer->RenderImGui(ImGui::GetDrawData());
}
