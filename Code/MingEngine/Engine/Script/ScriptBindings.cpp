...
namespace
{
void CallVoid0(Object* object, std::string const& className, std::string const& methodName)
{
	MethodBind const* method = ClassDatabase::GetMethodBind(className, methodName);
	if (method == nullptr)
	{
		return;
	}
...
void RegisterBridgeFunctions(asIScriptEngine* engine)
{
	...
	result = engine->RegisterGlobalFunction(
		"void __CallVoid(NativeObject@, const string &in, const string &in)",
		asFUNCTION(CallVoid0),
		asCALL_CDECL);
	GUARANTEE_OR_DIE(result >= 0, "Failed to register __CallVoid0()");
}
