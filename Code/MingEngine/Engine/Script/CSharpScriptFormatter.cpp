...
CSharpScript::~CSharpScript()
{
	...
	{
		g_engine->m_scriptSystem->RemoveScriptBridge(this);
	}
}
...
{
	if (owner == nullptr || !g_engine->m_scriptSystem->TryBeginScriptInstantiation())
	{
		return false;
	}

	if (!g_engine->m_scriptSystem->AddScriptBridge(this, GetVirtualPath().GetString()))
	{
		return false;
	}
...
CSharpInstance::~CSharpInstance()
{
	if (m_owner != nullptr)
	{
		g_engine->m_scriptSystem->UnregisterScriptOwner(m_owner->GetObjectID());
	}
