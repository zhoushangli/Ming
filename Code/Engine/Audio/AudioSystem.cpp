...
FMOD_VECTOR EngineToFMODTransform(Vec3 const& position)
{
	FMOD_VECTOR fmod;
	fmod.x = -position.y;
	fmod.y = position.z;
...
//-----------------------------------------------------------------------------------------------
void AudioSystem::BeginFrame() { m_fmodSystem->update(); }
...
)
{
	...
	if (channelAssignedToSound)
	{
		...
		float       frequency;
		FMOD_VECTOR position = EngineToFMODTransform(soundPosition);
